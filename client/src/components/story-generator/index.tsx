            ...
            <div className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-1 space-y-4">
                        <h2 className="text-lg font-semibold text-gray-900 mb-4">Story Settings</h2>
                        <div>
                    ...
                    {/* Results Panel - Takes 2/3 on large screens */}
                                    ...
                                    <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                                        <div className="flex items-center mb-4 justify-between">
                                            ...
                                            <Button 
                                                onClick={() => {
                                                    if (result.story) {
                                                        const storyText = `${result.story.Title}\n\n${result.story.Summary}\n\n` +
                                                            result.story.Chapters.map(ch => `Chapter ${ch.Number}: ${ch.Title}\n${ch.Content}`).join('\n\n') +
                                                            `\n\nThemes: ${result.story.ThemesOrLessons.join(', ')}`;
                                        ...
                                        {(result.executionTime || result.timestamp) && (
                                            <div className="text-xs text-green-600 mb-4">
                                                {result.executionTime && `Execution Time: ${result.executionTime}ms`}
                                                {result.timestamp && ` | Generated: ${new Date(result.timestamp).toLocaleTimeString()}`}
                                            </div>
                                        )}

                                        {/* Story Display */}
                                        <div className="bg-white rounded-lg border p-6 space-y-6 max-h-[80vh] overflow-y-auto">
                                            {/* Story Title */}
                                            ...
                                            <div className="space-y-6">
                                                <h2 className="text-lg font-semibold text-gray-700 border-b pb-2">
                                                    Chapters ({result.story.Chapters.length})
                                                </h2>
                                                
                                                {result.story.Chapters.map((chapter) => (
                                                    <div key={chapter.Number} className="bg-gray-50 rounded-lg p-4 space-y-3">
                                                        ...
                                                        <div className="space-y-3">
                                                            ...
                                                            <div>
                                                                <h4 className="text-sm font-medium text-gray-600 mb-1">Image Prompt:</h4>
                                                                <p className="text-sm text-gray-600 italic bg-yellow-50 p-2 rounded border-l-4 border-yellow-400">
                                                                    {chapter.ImagePrompt}
                                                                </p>
                                                            </div>
                                                        </div>
                                            ...
                                            {/* Themes and Lessons */}
                                            {result.story.ThemesOrLessons && result.story.ThemesOrLessons.length > 0 && (
                                                <div className="space-y-3">
                                                    <h2 className="text-lg font-semibold text-gray-700 border-b pb-2">
                                                        Themes & Lessons
                                                    </h2>
                                                    <div className="grid grid-cols-1 gap-2">
                                                        {result.story.ThemesOrLessons.map((theme, index) => (
